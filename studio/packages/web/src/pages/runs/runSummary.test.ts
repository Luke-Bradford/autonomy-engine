...
describe('deriveRunLifecycle', () => {
  ...
  it('tracks started → finished', () => {
    ...
    expect(deriveRunLifecycle(events)).toEqual({ status: 'success', waitingReason: null, cancelRequested: false });
  });
  it('maps run.interrupted', () => {
    const events = [envelope({ type: 'run.interrupted', runId: 'r', reason: 'boot' })];
    expect(deriveRunLifecycle(events)).toEqual({ status: 'interrupted', waitingReason: null, cancelRequested: false });
  });
  it('CX1 (#1320) — a cancel un-parks a waiting run in the live view, then its finish reads `cancelled`', () => {
    ...
    expect(deriveRunLifecycle(events)).toEqual({ status: 'running', waitingReason: null, cancelRequested: true });
    events.push(
      envelope({
        type: 'run.finished',
        runId: 'r',
    ...
    expect(deriveRunLifecycle(events)).toEqual({ status: 'cancelled', waitingReason: null, cancelRequested: true });
  });
  ...
  describe('#870 — mirrors the reducer’s park/unpark rules', () => {
    ...
    it('does not let an unpark event resurrect a terminal run', () => {
      ...
      expect(deriveRunLifecycle(events)).toEqual({ status: 'failure', waitingReason: null, cancelRequested: false });
    });

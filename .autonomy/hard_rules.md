...
- **Never modify the loop's own guardrails in an unattended session** — `.autonomy/**` (this pack:
  loop_prompt.md, hard_rules.md, config.yaml), `engine/bin/safe_merge.sh` (the merge gate), or
...
- **Never weaken a settled decision or engine invariant to pass a ticket** — fail-safe never
  ...
  floor, Python 3 stdlib only, repo-agnostic `engine/bin/`/`engine/lib/`, `merge_gate` safe default. A ticket that

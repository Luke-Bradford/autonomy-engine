...
them into a shared home is #977; until then the engine-scoped `lint-and-test` CI
job stays, because it runs the only tests covering them. **The non-negotiables below (bash 3.2,
...
- **Python 3 stdlib only.** No PyYAML, no yq, no third-party imports anywhere. Config parsing goes
  through `engine/lib/config_parser.py` (a restricted-YAML-subset parser, stdlib only). Adding a dependency
...
- **Every script's executable body is guarded** by `[ "${BASH_SOURCE[0]}" = "${0}" ] || return 0`
  ...
  defines functions. Adapter files that are functions-only (`engine/bin/agents/*.sh`) need no guard.
...
- **Reset-epoch split:** agent adapters (`engine/bin/agents/*.sh`) only *extract* the rate-limit reset
  epoch and return it in their outcome string; `engine/bin/supervisor.sh` is the sole writer of
...
- **Repo-agnostic:** no target-repo-specific values (GitHub owners, board titles, issue numbers)
  hardcoded in `engine/bin/` or `engine/lib/`. Everything repo-specific comes from the target repo's
  `.autonomy/config.yaml`. `engine/templates/` and `docs/` may use placeholders/examples — those are fine.
...
pipelines (the sequencer subsystem map — read before touching engine/lib/pipeline.py or the dispatch path).

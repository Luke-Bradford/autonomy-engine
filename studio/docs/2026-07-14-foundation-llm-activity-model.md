...
- **`agent_task`'s "any exit code is `succeeded`" (exit code is data) has exactly TWO carve-outs**,
  ...
  **The opt-in is per-ACTIVITY-TYPE since #816:** `quota.classifyActivityTypes` names which invocation shapes the
  ...
  WHICH SURFACE `agent_task` matches remains open (#816). A stdout TAIL cut was investigated and
  REJECTED: `bin/agents/claude.sh` classifies a Claude Code limit from a MID-STREAM `rate_limit_event`
  and treats a session that continues past one as not-blocked, and `bin/agents/codex.sh` is the same
  shape — so the signal is not positional in either CLI and a tail cut would miss real refusals. The
  evidence pointed at a better candidate, carried forward on #816: **narrow by SOURCE.** Both engine
  adapters parse ONLY structured error envelopes and neither greps agent content ("an error envelope is
  API/CLI output, never model content" — `codex.sh`). This adapter cannot do that today because it
  matches a flat text blob rather than a parsed event stream; it needs a per-CLI output-format contract.

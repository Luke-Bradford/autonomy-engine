...
export interface ExternalWaitCompleter {
  complete(token: string, body: Buffer | undefined): Promise<ExternalWaitOutcome>;
}
...
export function createExternalWaitCompleter(deps: DriveDeps): ExternalWaitCompleter {
  ...
  return {
    async complete(token: string, body: Buffer | undefined): Promise<ExternalWaitOutcome> {
      ...
      const result: { verdict: ExternalWaitOutcome; record: RunEvent | null } = deps.db.transaction(
        (): { verdict: ExternalWaitOutcome; record: RunEvent | null } => {
          ...
          if (!checked.ok) return { verdict: 'invalid_payload', record: null };
      ...
      if (result.record === null) return result.verdict;

...
- **Container timeout mid-drain: a defect, fixed here.** Measured before the fix: with a loop's child
  still in flight after the cancel, the timeout alarm was _fresh_ (the loop is still `active`). It
...
1. **Confirmation copy for a run with an in-flight side-effecting node** (an `http` POST, a `copy`
   ...
   **Settled in CX4:** the confirmation always says _"Work already sent (a request made, rows
   written) is not undone."_

...
| # | Ticket |
| --- | --- |
| **A0** | **Branch/outcome model** (PREREQUISITE). **The `Edge` union is now BUILT in #1 (F1)** — `BranchEdgeSchema` = `{on:'branch', branch:'<name>'}` (label required; may also be a capped `back:true` arm), operational outcomes stay `success/failure/skipped/completion`. A0 no longer designs the schema; it must (a) implement `if`→`true`/`false` and `switch`→named case/default against it, (b) carry the chosen branch label into run state via `condition.evaluated`/`switch.evaluated` (#1 F1 deliberately did NOT invent a rival mechanism — `edgeState` currently returns `unsatisfied-terminal` for every branch edge and `settle` emits a diagnostic saying so), and (c) REPLACE `validateDoc`'s blanket branch-edge error with the real rule ("a branch edge's source must declare that branch"), which needs the ActivityDefinition contract. **NB `validateDoc` IS a gate — this row's original claim that it was "advisory" and that "the server never validates" was already false when #444 wired it, and is corrected here (#786).** It runs on the WRITE path in `createPipelineVersion`, the single funnel for the save route, git import and workspace-apply alike, and the canvas badge calls the same function so the two cannot drift. What it is NOT is a READ-time check (`pipeline-versions.ts`: "WRITE-PATH ONLY, deliberately: reads never validate"), so a version minted BEFORE a given rule existed still reaches the reducer carrying whatever that rule would now refuse — which is why the reducer's `noteInertBranch` diagnostic still earns its keep, and A0 should retire it only once it has something better for those legacy docs. |
...
| A9 | `execute_pipeline` first-class (surface `call_pipeline`). **Carries #516's requirement (closed into this row 2026-07-22):** when real child-run execution ships, a gone **or present-but-unparseable** child pipeline version (`DocUnresolvableError`, which per #515 covers both) MUST be **classified at the `startChild` seam** — mapped to a typed `call.returned{failure}` / `node.failed{kind:'permanent'}`, or terminalized by the child run's own boot reconcile — NEVER an escaping throw: in a boot re-drive that throw would escape #508's resolve-site guard (`run/reconcile.ts`, which covers only the parent's own version) into #479's per-run catch and re-file the parent `failed` on every boot forever. **#516 SATISFIED 2026-08-12 by #796 (P3b slice 1)**, ahead of A9's authoring half: the `startChild` branch now resolves the child version through `createChildRuns.ensure`, which catches the resolver's throw and returns a TYPED refusal the executor yields as `call.returned{failure}` — alongside refusals for a cross-owner callee, an archived child pipeline, non-replay-safe params and a run-time call-depth breach. Nothing throws out of that branch, which is the property #516 asked for. (The prior note here — "verified unreachable pre-A9 … no server code creates a `parentRunId` run" — described the P3a stub and is now historical.) **#796 item 3 (2026-09-25):** a refused spawn's `call.returned{failure}` now carries `reason`, shown in the run page's Failure section. **#796 item 2 (2026-09-25):** `call.wait: false` is READ — ADF's `waitOnCompletion: false`. The node succeeds on a new `call.detached` event once its child exists and has been kicked, with no outputs (the save gate refuses declared outputs and `${}` refs to them); a spawn refusal still fails it. The child runs to its own end, including across a restart after its parent finished: the reactor returns nothing to a detached parent, #1053's `parent_terminal` freeze and #1041's never-started sweep both exempt it, and "detached" is read from the parent's bound DOC because the executor kicks before the detach is durable. Absent `wait` still waits, and the canvas checkbox now reads checked for it. CATALOG_VERSION 31. |
...
| **A16** | **`webhook` typed-output + callback contract (Round-3, HITL).** **SPLIT — inbound half LANDED, outbound half deferred.** The **inbound** half (built): the inbound authed/replay-protected callback payload is validated at the HTTP boundary against the webhook's declared **`config.outputs`** (reusing the generic F13 contract — no separate `outputSchema` field, since `validateRefs` already type-checks `${nodes.webhook.output.decision}` against `config.outputs`) → carried on `externalWait.completed.outputs` so the ref resolves downstream (ADF `reportStatusOnCallBack`). Unblocks human-approval: the callback URL is retrievable via `GET /api/runs/:id/external-waits`. **OWNER-SIDE SURFACE AS BUILT (2026-08-01, #900):** "retrievable" meant retrievable by an API caller only — nothing in the web app called that route, so the Monitor said `waiting (callback)` and offered no way to find the URL, and the human-approval case this row exists for was reachable only through `curl`. The run monitor now lists each pending wait with its parked node named, its expiry, and the declared-`config.outputs` contract its body must satisfy (read off `outputContract`, so the description cannot drift from what `checkInboundOutputs` enforces); the path is revealed on demand because the token in it is a capability, not an identifier. **COMPLETING THE WAIT AS BUILT (2026-08-05, #901):** the row above deferred this because the only primitive was the anonymous callback route, whose fail-closed identical-404 discipline is right for an untrusted prober and actively unhelpful to the run's owner. It is now a SECOND DOOR onto the same settle, not a second settle: `POST /api/runs/:id/external-waits/complete` is owner-scoped through the run (`requireOwned`), RE-DERIVES the capability token server-side — so the token never reaches the browser at all, which is the reason this exists rather than the SPA POSTing the revealed path — and hands it to the very completer the anonymous seam uses. The two differ only in DISCLOSURE, which is a property of the CALLER rather than of completing: an owner is told `already completed` (409), `expired` (410) or which declared field failed (422 `external_wait_payload`, through the central error handler so the reason survives), while the anonymous seam keeps collapsing every one of those to an identical 404. The request body pins `attemptId` as a CAS basis (#904's shape), so a body composed for the attempt the operator was looking at is refused rather than silently settling a re-parked successor; `payload` is required with NO default, so an absent body cannot be manufactured into "completed with no outputs". The monitor grows a **Complete wait** editor beside the URL, whose draft is held ABOVE the wait-epoch remount so an unrelated wait settling cannot discard half-typed input. The anonymous seam is behaviourally UNCHANGED (it still awaits the resumed drive; the owner route answers 204 the moment the wait is durably settled and drives in the background, per the rerun route's convention). Its non-conforming `422`/`404` error bodies are deferred, deliberately: its clients are external callers, not `apiFetch`. A webhook's declared outputs rendered NOWHERE in the drill-in — the monitor's fold set `outputValues` only from `node.succeeded`, which a parked node never emits, and the panel gates its whole Outputs section on that field — so the payload this row's whole inbound half exists to deliver was invisible on every completion, `curl` or in-app, since A16. Found while building this, filed as #911, FIXED 2026-08-05: `externalWait.completed` now records its `outputs` onto the row (defaulted to `{}` for a pre-A16 event, agreeing with `onExternalWaitCompleted` rather than inventing a third answer), and the e2e asserts the value on the PANEL instead of off the raw event log. The **outbound** half (deferred → follow-up issue): injecting a `callBackUri` + correlation token into an OUTBOUND trigger would require this `kind:'control'` node to perform outbound HTTP I/O, against the #1 D6 no-connector-I/O invariant — it needs its own control-vs-execution disposition and does not block the HITL use-case. |
...
- **Activity-level drill-in** — per-node input/output/error/usage/cost (#2 metering), retry
  ...
  still-unbuilt slice (U27). Input shipped 2026-09-26 (#890, `node.dispatched.input`, with the applied parameters on `.params` — see the UI spec's U24 AS BUILT rows).

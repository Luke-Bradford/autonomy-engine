...
| **L14** | **`cli`/subscription connection kind (T5):** a CLI-agent connection `llm_call` accepts + single-shot adapter (`claude -p`/`codex exec` → stdout); quota/reset-window primitive; `meteringStatus` metered/unpriced/unknown + run-cost completeness flag. **DELIVERED — the quota/reset-window primitive (L14c) spans BOTH invocation shapes on an `agent_cli` connection, not just `llm_call` (#799).** The optional per-connection `quota.exhaustionPattern` reclassifies a matching non-zero exit as `rate_limit` + `retryAfterSeconds` and excludes it from metering, on `llm_call` and `agent_task` alike; either shape can therefore ARM the persisted per-connection window that the dispatch-time admission gate reads. The gate itself always keyed on connection KIND, so it gated both shapes from the start — it was the WRITE side that only `llm_call` could reach, which left a connection consumed solely by `agent_task` nodes unable to ever learn it was exhausted. **#816 then made the opt-in per-SHAPE** (`quota.classifyShapes`, absent = both, `CATALOG_VERSION` 18→19): the two shapes match very different surfaces — a single completion vs the agent's own transcript — so an operator can scope `agent_task` out without disarming `llm_call` on the same connection. Scoping governs the WRITE side only; the gate still keys on connection kind. | 1 |
...
- **`agent_task`'s "any exit code is `succeeded`" (exit code is data) has exactly TWO carve-outs**,
  ...
  **The opt-in is per-SHAPE since #816:** `quota.classifyShapes` names which invocation shapes the
  ...
  WHICH CHANNELS/POSITIONS `agent_task` matches remains open (#816) — matching only a stdout TAIL was
  tried and refuted, because `bin/agents/claude.sh` classifies a Claude Code limit from a MID-STREAM
  `rate_limit_event` and handles a session that continues past one, so a tail cut would miss real
  refusals while leaving the likelier false positive (the agent's closing summary) in place.

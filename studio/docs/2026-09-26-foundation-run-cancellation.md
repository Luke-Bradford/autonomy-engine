...
- **The node words depend on the run.** Under a `cancelled` run, a node or container left non-terminal keeps its live status in the log (D2/D4, and a parked node under D5). Its word changes, but its tone does not. It reads "not run (cancelled)" if it never started and "stopped (cancelled)" if it was live. `nodeStatusLabel`/`containerStatusLabel` take the run status, and the table, the drill-in panel and the graph all pass it, so the three surfaces keep one vocabulary (U25).

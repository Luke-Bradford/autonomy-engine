...
*(Since closed: item 4 by #1196, which
lifted the policy into `sink-columns.ts` when postgres became the second sink; item 2 by the
describe seam itself, which now describes a `query` end by name exactly as it does a `table`
end — `.columns()` in `sqlite.ts`, the statement describe in `postgres.ts`; item 3 as below.)*

### As built — the store's own `NOT NULL` (#1162 item 3) — a STATED BEHAVIOUR BREAK

§6.2's refusal (`onError: 'null'` onto a NOT NULL column) used to read only the sink dataset's
DECLARED `nullable`, which §7 says is deliberately not the gate. A column the dataset calls
nullable but the store declares NOT NULL therefore reached the store and failed there, as a
constraint violation mid-copy. It now runs twice, as ONE rule (`refuseNullOnNonNullable`,
`sink-columns.ts`) with two evidence sources: at dispatch against the declaration (unchanged), and
inside each database sink against the real schema, under the write lock and before an overwrite's
`DELETE`. **The break:** a copy with `onError: 'null'` into such a column is now refused up front
even when no value would in fact have failed to coerce — the break the declared half always made,
now extended to the real schema.

The store half refuses only where the store CERTAINLY refuses an explicit NULL, because `notnull`
alone over-refuses. Measured: SQLite's `INTEGER PRIMARY KEY NOT NULL` (a rowid alias),
`NOT NULL ON CONFLICT REPLACE|IGNORE`, and a `BEFORE INSERT` trigger all report `notnull = 1` and
accept NULL; so does a postgres `attnotnull` column behind a `BEFORE INSERT … FOR EACH ROW` trigger
(on the table or any partition), a `DO INSTEAD` `INSERT` rule, or a foreign-table partition — but
not a disabled trigger or a `DO ALSO` rule, behind which the constraint still fires. Those shapes
are exempt, deliberately broadly, and keep the old behaviour. The exemptions only ever leave out
columns the rule could have refused, which is the direction §7 allows.

The declared half is kept as it was. With the store now checked, a dataset that declares a column
`nullable: false` over a store column that is actually nullable is refused on the declaration
alone. That is §7's "stale declaration blocks a copy that would succeed" shape, but §6.2 specifies
this refusal against the declaration, so retiring the declared half is a separate decision, not
part of this item.
...
| **M8**  | The mapping authoring panel (§13). **SPLIT IN TWO** — slice 1 (#1169) is the `objectList` PRIMITIVE (§13's *"build it as a primitive rather than a copy-specific panel"*): the derived row control, which upgrades `copy.mapping` and `llm_call.tools`. Slice 2 is the copy-specific half — Auto-map and the explicit *unmapped* state (#1170), then per-column expressions (#1178), which needed the picker to probe a placement rather than a field name. **DONE.** See §13's as-built block                                                                                                                                                                                          | UI epic; e2e-gated                                                                                                                                                                                                                        |
...
**Item 3, the per-column expression escape hatch, SHIPPED (#1178) — §13 is complete.** The flyout
now probes a PLACEMENT rather than a top-level field name (`PickerTarget` in `ConfigFieldControl.tsx`):
the control that knows its position builds the candidate config, and the panel only runs the
whole-doc validator. A cell places the candidate into the list's DRAFT rows, read back cell by cell
as an apply would, because the row may not exist in the stored doc yet. Its baseline is the issues
present both with the cell EMPTY and with it holding a plain LITERAL: an issue both share is not
about the cell's content. Each half is load-bearing, and the docblock names the cell that breaks
without it.

Candidates are now filtered in INSERT mode too, not only REPLACE. The old argument ("a template always
resolves to a string, so nothing can be type-refused") holds for types and fails for a field that
refuses `${}` outright. `source`/`sink` are held to a literal by this section, so both mode probes
carried that refusal equally, the field read as a template, and an unfiltered list would have
offered only references Save refuses. So a column-name cell's flyout comes back empty and says so,
and so does an `expression` on a row that already reads a `source` (the XOR). No cell-name table
decides which cells are offered anything; the validator does. The same holds for the other row

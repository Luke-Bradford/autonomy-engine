...
| | **AS BUILT (2026-09-25, #864 item 1 — container fields).** A loop's `exitWhen` and a foreach's `items` get the flyout through the same `ConfigFieldControl`. `availableRefs` takes a `RefSite`: a node, or `{ kind: 'container', containerId, field }`, each answered from the `ScanScope` its validator builds (`exitWhenScope` / `foreachItemsScope`), so the offer and the refusal come from one derivation. `items` gained the array TYPE check `exitWhen` already had for booleans: without it every `run.*` field would have been offered, saved clean and failed at `evalForeachItems`. For the same reason `items` offers no `default()`-wrapped reference (its `""` fallback is never an array). Still open on #864: a filter's per-field `${item}`, the FUNCTIONS half, and `json` fields. The node issue list is #863. | |
| | **AS BUILT (2026-09-25, #864 item 3 — the FUNCTIONS half).** A second toggle, **Wrap in function**, beside Insert reference on every field that has the flyout. A function is put AROUND the expression the caret is in, never inserted bare: `${name()}` would be refused at save the moment it landed. The target is the author's selection when it lies inside one `${}` body (so a sub-expression wraps in place), otherwise that expression's whole body; a caret in no `${}` gets a hint, not a list. The **no-false-offer** rule holds by the same mechanism as references: every catalog function is tried around the target and kept only if the whole-doc validator raises nothing the field's CURRENT text does not already raise — so arity (`substring`, `default`), argument types and a type-checked field (a `filter`'s `items`) are decided by the save gate, not restated. Rows read `name(args) → ret` from `fnSignature`, printed off the same `FnSpec` the checker reads. The span is located by `refAt` in `shared/engine/expr.ts`, beside `scanTemplateRefs`, translating offsets through `protectEscapes`' sentinel (not the length of the `$${` it replaces). Still open on #864: a filter's per-field `${item}` and `json` fields. | |
| | **AS BUILT (2026-09-26, #863 — the NODE ISSUE LIST).** Every validation issue is attributed to the element it is ABOUT, on the plain strings, without waiting for R3. `issueSubject` (`containerRules.ts`) reads only the message's LEADING location — `node.<id>`, `nodes.<id>.`, `container.<id>.`, or a quoted `node`/`container`/`edge`/`back-edge`/`duplicate node id`/`container id '<id>'` — through the same location patterns `readableIssue` uses, so the two readers cannot disagree about where an id ends. An id later in the sentence (a `${nodes.X…}` producer, `child '…'`) is never the subject. An unquoted capture is widened against the doc's real ids, because ids may contain `.`. The canvas draws a count badge on the activity or container (absolutely positioned, so no box resizes; on a container it is `aria-hidden` and the count joins the box's accessible name), and outlines it in `--error` when it is not selected. The node, container and edge panels list their own issues. The node panel COUNTS but does not repeat its own policy refusals, which `PolicyEditor` lists beside their fields. Issues with no single subject (a param default, a forward cycle, a call-graph refusal) have no badge. The canvas's full badge list still carries every issue, so that list is the pipeline-level bucket. No new live region (#1249). R3/U8b would replace the string parse with a field read; nothing here needs it first. |
...
| | **AS BUILT (2026-08-14 — #1087).** Create/edit shipped with U13a, but the `Kind` dropdown changed NOTHING below it: the whole `config` was one raw `Config (JSON)` textarea, `ConnectionSchema.config` is an untyped `z.record`, and `routes/connections.ts` validates no key — so binding a connection meant knowing each kind's keys by heart, and a misspelt one saved cleanly and failed at DISPATCH. The per-kind schemas turned out to EXIST already, as module-private consts inside the five adapters, re-parsed at dispatch and invisible to the web. So this was a consolidation, not an invention: they moved to `shared/catalog/connection-config.ts` (the pattern `fs-activity-config.ts` set for activity configs after the same drift) keyed by kind in a `Record<ConnectionKind, …>`, so a new kind without a schema is a TYPE error; each adapter's `configSchema` is asserted by IDENTITY to be that same object, which fails even for an equivalent re-declaration. The form then derives its controls through the EXISTING `deriveConfigFields`/`ConfigFieldControl`/`assembleConfig` engine that `ContainerPanel` already proved works off-canvas. Byte-identical shapes, so `CATALOG_VERSION` is deliberately not bumped. **Three decisions worth keeping.** (1) It is ADVISORY, never a gate: the server stores an `agent_cli` with no `command` today, so a client-side refusal would make an existing row unsaveable after an unrelated rename — the kind's own schema is run and its issues are SHOWN, and Save still works. (1b) The advisory is `connectionConfigAdvisory` in shared, NOT a bare `safeParse`, because the shared `fs` schema deliberately cannot carry the absolute-root check (below) — a schema-only advisory would have left the one path-safety-relevant key in the whole catalog as the only one the form said nothing about. It uses a permissive browser-safe predicate that accepts POSIX and Windows-drive forms, so it never warns about a path the server would accept; the server's `node:path` check remains the authority. (2) The one server/shared divergence is `fs`'s absolute-root check, which is `node:path`'s platform-aware `isAbsolute` and cannot live in a browser-safe package; the adapter refines the SHARED `roots` rather than re-declaring it, so the `.min(1)` and its message stay in one file. A pure `startsWith('/')` in shared was rejected — it would change a security-relevant path check for zero benefit to the form, since a `.refine()` is a check, not a wrapper, and is invisible to field derivation. (3) Nothing is silently dropped: a key no kind declares survives a field-mode save untouched (`assembleConfig` merges over the original), and a key ANOTHER kind owns is rendered as a CARRIED field so blanking it is the repair — `ContainerPanel`'s rule for illegal container fields, and a dead end without it. `fs` post-dates this row's kind list; it is covered because the `Record` exhaustiveness forces it. **Secret entry says what the kind DOES with a secret** (`CONNECTION_SECRET_USE`, beside the schemas) and whether it REQUIRES one (`connectionKindRequiresSecret`, never a re-spelt `kind === …`) — but the input is never `required`, because on edit blank means keep the stored secret and on create the server accepts a secretless row. |
...
| | **AS BUILT (2026-08-14, slice 1 — #1085):** the OVERVIEW only. `/` lists the newest 5 runs (status word from the Monitor's one vocabulary, pipeline name + version, absolute start time, each linking to its run detail) above the hub signposts, which survive because the rail and Home naming the same hubs is asserted by `App.test.tsx` and `e2e/hub-nav.spec.ts`. A PREFIX, not a census: no counts or totals anywhere on the page, and no way to extend it — Home fetches ONE page and ignores `hasMore`, for the reason `RunsPage`'s origin tabs render `12+`. `listRuns` gained an optional `pageSize` so the five reach the wire as `limit=5` rather than being sliced off a 50-row response: the server aggregates metered per-run costs for the rows it returns, and `/` is the router's CATCH-ALL, so an over-fetch would bill a 50-run aggregation on every stray URL. Settings (`#/settings`, theme + master-key status) is slice 2 — built next, see the row below. | |
| | **AS BUILT (2026-08-14, slice 2 — #1094):** `#/settings`, reached from a gear in the rail FOOT (not a hub: no secondary pane, and it is absent from the `hubs.ts` SSOT by design). Two sections. APPEARANCE re-renders the rail's own `ThemeToggle` against the singleton store — the second consumer that component's docblock had been waiting for, so the two switches show one value rather than forking the state; `e2e/support/theme.ts`'s `setTheme` is scoped to the rail because the accessible name "Dark mode" is now ambiguous on this route. SECRET ENCRYPTION is why the page exists: `GET /api/settings` reports the master key's PROVENANCE (`env` \| `file` \| `generated`, plus the absolute path of the key FILE where one exists — never key material), and when the key was AUTO-GENERATED the page carries the back-it-up advisory that until now went to `console.warn` and the server log and nowhere else. That default first-run path silently mints the key encrypting every stored secret, and losing the file makes all of them permanently undecryptable with no recovery beyond re-entering each credential — so the one instruction that prevents it was being printed to a log an operator running this as a service never opens. Read-only by design: rotation/re-keying is a re-encrypt-everything operation and is not this ticket. | |
...
| | **AS BUILT (2026-08-22, #1231 — slice 1: the Monitor child-run DRILL).** This row is four things and two of them had already shipped out of band: the target-pipeline picker and the param-map landed with #425/#953 (`pages/pipeline/CallPanel.tsx` — pipeline + version dropdowns, an expression-mode target, per-param typed rows). The row was blocked on #796, whose spawn seam landed 2026-08-12, so what was left was real. **#796 and #735 together made a child EXECUTE and made its parent SAY SO — the call node sits `waiting` for the whole time the child is in flight — and neither gave the operator a way to LOOK at it.** `NodeActivity.childRunIds` had exactly one consumer, `RunCostSummary`'s `ExcludedChildren`; `RunDetailPage.tsx` did not contain the string `parentRunId` at all. So a composed pipeline ran correctly and was unreadable in **both** directions: a call node that named no child, and a child run indistinguishable from any other run with no way back to whatever called it. The runs list's `Child` tab could say a run WAS a child and never whose — and its own hint still read *"(not yet produced — see #796)"*, a live falsehood on the operator's strip ten days after the seam landed, corrected here. **The drill is a SECOND site, not a shared component with `ExcludedChildren`, and that is the decision worth keeping.** The two render the same ids and agree on nothing else: the cost section's sentence is a claim about MONEY (*"this figure excludes those runs, and anything they called in turn"*) which a node panel must not carry, so sharing would mean dragging a spend claim into the drill-in or adding a wording-switch prop — and the accessible names have to differ ANYWAY, because both render on this one page and two links named the bare run id are ambiguous to any reader addressing controls by name, a screen reader's and a spec's alike (`Child run <id>` / `Parent run <id>`, on `RunsPage`'s `Watch run <id>` precedent). **The source is the FOLD, and for a NODE-scoped section the query is not a weaker source but an incapable one:** `RunSchema` carries `parentRunId` and no call-node id, so `GET /api/runs?parentRunId=` can say a run is a child of this RUN and never which node spawned it. It is also the *safer* one, inverting the polarity `runSummary.ts`'s own docblock states for SPEND — `call.returned` echoes a `childRunId` for a spawn `child.ts` REFUSED, a run that never existed, and this array is filled only from `call.started`, which is appended after the child's row, so it can never offer an id that 404s. **THE STATE A `length > 0` GATE SHIPS SILENT**, and the one this ticket is about: the reducer parks the node on the `startChild` COMMAND and the announcement lands only once the child's row exists, so a node reads `waiting` with NO id — transiently on every live spawn, and permanently if the server died in between (#1041's `pending` orphan). Rendering nothing there says "no children" about a node parked on one, so that case gets its own sentence, on the panel's existing convention for the Duration and empty-Outputs absences. **Deliberately NOT gated on `stream.replayComplete`**, where the cost section on this same page deliberately is, and the difference is what each surface CLAIMS: a truncated replay makes both under-count, but the cost section would then print a total wrong by an unknown amount — the manufactured authority #473/F13a forbid — where this one shows fewer links, a missing way down rather than a false statement; and the live park is exactly when the link is wanted. The wording is **tense-neutral** for the same reason it is a list: `childRunIds` is append-only and never cleared, so it outlives the park, holds finished children as readily as live ones, and carries no child STATUS at all — a `skipped` call node (a container timeout via `abandonLiveChildren`) can be holding one that is still running. It may say what a child IS, never what it is DOING. **Two knowing residuals.** An RS1 rerun's copied call node lists nothing: `run.reseeded` refuses to re-derive a child id, so the panel shows the child's outputs with only the "reused from run R1" hint as a route up — correct (this run spawned nothing) and worth naming so it does not read as a bug. And `Rerun of`, the row directly above `Called by`, is still a `navigate`-on-a-button while the new row is a `<Link>`; converting it edits a control RS2's tests pin BY ROLE in a PR about call lineage, so it is **#1232** rather than folded in. **DEFERRED — slice 2, the call-graph VALIDATION half:** `validateCallGraph` runs only on the server save path, because `canvasDoc.ts` calls `validatePipelineDoc` with no `selfId`/`resolvePipeline` and it returns `[]` immediately — so a cycle or a depth violation reaches the author as a rejected save, never as a live node-anchored badge. That needs a resolver seam the client can drive (and inherits #1011, whose save-time walk skips `${}` targets), so it is its own slice. `ExpressionPicker` in `CallPanel` landed separately as **#1012** (2026-09-25: the target and typed argument rows, placed into `Node.call`; the params-JSON box deliberately has none); cancelling a live child with its parent stays #796's remainder and the U28 cancel epic's (closed by CX3, #1320: a parent's cancel or end now cancels its live, non-detached children). | Author/Monitor |
...
| | **AS BUILT (2026-09-25, slice 4 — CONTAINER DUPLICATION; #935).** A loop or foreach with a body of several activities could only be rebuilt by hand. `duplicateContainer` copies the box, its whole body and the edges inside it in ONE undo entry, from a **Duplicate container** button in the container's property panel (not on the box header — the canvas is deliberately quiet, #992/#997) and from ⌘D when a container is selected. **It is not a second cloning path**: `cloneNodesInto` now takes the containers to copy alongside the nodes, under ONE id map, so every slice-3 rule applies unchanged and a container is just another endpoint — a loop's `exitWhen` (scoped to its own children) follows the copied children through the same remapper, an edge INTO the box is re-derived onto the copy (which is what keeps a foreach copy's `items` upstream in scope), and edges OUT of the box or its body are not copied, for the reason a node's are not (a back-edge from the body to an outside node is therefore not copied either). The one membership exception is written into rule 4: a node whose container is copied with it belongs to the COPY, where re-deriving from live containers would have put it back into the original. **Placement is the part with no precedent**, because a box is derived from its children and two overlapping boxes make it unreadable which activity belongs to which: the copy goes clear of every node in the horizontal band it will occupy, not merely of the original, or a second ⌘D would stack a box exactly on the first. That lands it off-screen whenever the row is wider than the pane, so the #785 reveal now also pans to a container that has just APPEARED — held pending (`revealReady`) until its box is derived from its children, because for one render the store holds the new nodes before the view does and the box sits at the empty fallback; the copy was previously visible only by that transient accident. ~~**Still deferred on #935:** ⌘C/⌘V of a container (the clipboard carries nodes only), and ⌘X.~~ Built in slice 6, below. (Cross-pipeline paste: slice 5, below.) |
| | **AS BUILT (2026-09-26, slice 5 — CROSS-PIPELINE PASTE; #935).** A copy made in one pipeline was refused in every other, so reusing a working chunk meant re-authoring it. It now pastes, under ONE refusal and two changes to the local rules. **The refusal:** a copied node that READS (`${nodes.<id>…}`, anywhere in the node — config, `call`, connection/dataset params) a node or container it did not bring is refused, naming each one. Within a pipeline such a read is right (the upstream still exists); across pipelines it names nothing, or — the reason this refuses rather than leaving it to the validation badge — a node that merely SHARES the id, because ids coincide across imported and seeded docs (`a`, `b`), and that doc validates, saves and runs while reading the wrong node. So the set checked is the source's node and container ids AND the target's, minus the copied set: a ref the source left dangling can still land on a target id. `referencedNodeIds` answers it with the SAME scanner and walk `remapNodeRefs` uses (an identity map plus a record of what matched), so "what a ref is" cannot differ between the check and the remap. `${params.x}` is deliberately NOT refused — a param is a by-name interface and a missing one is reported by the badge — and a copied call to the target's own version is refused at save as a call cycle. **The two rule changes:** re-deriving external in-edges and container membership (rules 3-4 of `cloneNodesInto`) is OFF for a foreign paste, since both look the copies' SOURCE ids up in the target's graph and would wire the target's own edges into them; and the copies land BELOW everything in the target, left-aligned and keeping their own layout, since source coordinates routinely put them on top of the target's nodes. The notice says `…from another pipeline`. ~~Not built: panning to a paste that lands below the fold (the #785 reveal pans to containers only).~~ **FIXED 2026-09-26 (#1336):** the reveal also pans to activities that appear already selected (a paste or a ⌘D duplicate) when every one of them is wholly off screen, and a foreign paste now clears the lowest node at its port-derived height rather than a flat 52px. ~~**Still deferred on #935:** ⌘C/⌘V of a container and ⌘X.~~ Built in slice 6, below. |
| | **AS BUILT (2026-09-26, slice 6 — CONTAINER CLIPBOARD + CUT; #935, which this completes).** **A container on the clipboard.** ⌘C on a selected box, or **Copy container** in its property panel, copies the box, its whole body and the edges inside it — the same `containerCopySet` ⌘D uses, so the two cannot disagree about what "the container" is. A paste in the same pipeline lands where a duplicate would (`duplicateContainerShift`, not the stagger, which would overlap the original box) and selects the copy; a paste into another pipeline follows slice 5's rules, and the refusal now also walks the copied CONTAINER, because a foreach's `items` is evaluated outside its box and so reads an upstream the copy does not carry. **⌘X** (and **Cut selection** in the multi-selection panel) copies the selected activities and deletes exactly those, with the edges that cascade with them, in ONE undo entry — never a selected edge between two unselected nodes, which a clipboard cannot carry back. **The load-bearing part:** a paste re-derives a copy's in-edges and container membership from the LIVE graph, and after a cut there is nothing live to derive them from. So a cut node came back with no upstream while its `${}` still read one — "Pasted", then refused at save. The clipboard now carries a `CutContext` (the removed in-edges and each node's container), used ONLY for a source whose original is gone from the live graph, only within one pipeline, and still through the connect gate, which drops an edge whose source was deleted since. An original brought back by undo is re-derived from the live graph like any copy. **A container is never cut** (said, not silently ignored): its only delete, the confirm-gated ✕ (#748), keeps the body and lifts it to the top level, so there is no delete a cut could be, and a new "delete box and contents" would be a second container-delete path. **Not done:** a cut node's DOWNSTREAM readers are not repointed at the paste (a paste is a new node; out-edges are never copied, as for any copy) — the validation badge names them. |
...
| | **AS BUILT (2026-09-26, #890 — the INPUT slice):** `node.dispatched` gains an optional `input` — the node's config after `${}` substitution and before any `{$secret}` marker is resolved, as compact JSON text bounded to `DISPATCH_INPUT_MAX_CHARS` (4k UTF-16 units, surrogate-safe, `chars` = whole length, `truncated` when cut). No secret plaintext can reach `${}` output (secret params are stripped, connection/sink/config-sink secrets decrypt after it, an upstream secure output folds as its marker), and the executor still scrubs every plaintext it resolved from the VALUE before stringifying. A node with `secureInput` OR `secureOutput` stores the marker (`secure.ts`, the `activity.captured` either-flag rule). An `llm_call` records it only under `capture: 'full'`, because its input is the prompt and #605's default promises no prompt text. The drill-in's **Input** section shows it with the foreach item it belongs to; a node the executor never dispatches (control activities, a pipeline call, a rerun-copied node) has none and gets no section. Resolved connection/dataset PARAMETERS are not in it: they are recorded beside it, see the next row. | Monitor |
| | **AS BUILT (2026-09-26, #890 — the PARAMETERS slice):** `node.dispatched` gains an optional `params`, the connection and dataset parameters the dispatch APPLIED over the stored settings (`captureDispatchParams`), as JSON keyed by the doc fields the author wrote (`connectionParams`, `datasetParams.source`/`.sink`). It records what was applied, not what was authored: a paired node's `connectionParams` are dropped (the executor never applies them), and a dataset end is recorded only if it resolved, so a source-only activity's inert sink parameters are absent. It is on the same terms as `input` in every respect: a separate 4k bound, a value-scrub against every resolved plaintext, the marker on a `secureInput` OR `secureOutput` node, and withheld from an `llm_call` not on `capture: 'full'`. That last is because a free-form setting (`agent_cli` `args`, an `http` header) can be bound to upstream text, which could be the prompt. A node that bound no parameters records none, never an empty object. The drill-in shows them under a **Parameters** heading inside Input, and says so when the config itself was not recorded. `input` stays the config only. | Monitor |
| | **AS BUILT (2026-08-01, #866 — the COST + TOOL-CALLS slice):** the drill-in states what a node SPENT and which tools its loop ran. Neither needed new data — `activity.metered` has carried provider/model/tokens/`costEstimate` since L2, and `activity.toolCalled` carries `toolName`/`round`/`callId`/`isError` IN THE CLEAR (only args/result are reduced to chars+hash) — so the work was almost entirely HONESTY, because a per-node money figure misleads where a per-run one does not. Three readings are pinned: `formatUsd` never rounds a real charge to `$0.00` (below its smallest stateable figure it renders `< $0.000001`); `readNodeCost` classifies a node into `none`/`covered`/`unknown`/`lower-bound`/`exact` BEFORE a dollar sign is drawn, so a run of unpriceable exchanges shows no number at all rather than "at least $0.00"; and a `tokenReportedResponseCount` separates "counted zero" from "nobody counted", so an `agent_cli` node — whose `cliSpendFact` carries no token counts — reads `not reported` instead of `0 in · 0 out`. The metered categorisation was EXTRACTED into a shared accumulator rather than copied, following `rollupFromAggregates`' one-derivation-site precedent, and the fold rides the EXISTING `deriveNodeActivity` walk (#849 — the page already folds its log three times a frame). `round` is not unique on its own, so each call carries the ATTEMPT and the foreach INSTANCE. **Deferred:** the prompt/completion surfaces (still redacted shape only, gated on #605/L9b — since SHIPPED for a `capture: 'full'` node, see the #605 row below), the drill-in from the run CANVAS and a rerun's copied frontier (#866 items 3-5 — the parked-webhook half of that list SHIPPED 2026-08-05 per the #911 row below, and the copied-frontier half SHIPPED 2026-08-05 per the #918 row; only the canvas drill-in remains); and the tool table's e2e coverage, which has no egress-free producer (**#892**) — the cost half does, via an `agent_cli` connection pointed at `/bin/echo`. | Monitor |
| | **AS BUILT (#605, L9b slice 1 — PROMPT/COMPLETION).** A `capture: 'full'` `llm_call` stores its prompt and completion TEXT on `activity.captured` (server-budgeted: 16k per field, 64k per exchange, a cut field says so), and the drill-in's `CaptureSection` renders the most recent 3 exchanges as disclosures — system, each turn, completion — stating a cut, an empty completion, and a missing one as three different facts. A secure node's text arrives as `SECURE_REDACTED` and the section says why under either flag. A default (metadata) node still shows no section: its capture is lengths and hashes. |
...
| | **AS BUILT (2026-08-13, #1075).** `GET /api/workspace/audit` (#3 G6a) shipped with the substrate, its writers, owner scoping and keyset pagination — and **zero web callers**, so every `repo.connected`, `pipeline.archived`, `pipeline.restored`, `import.applied` and `pipeline.published` was written durably and read by nobody. `WorkspaceEventSchema`'s own docblock had already stated the intent (*"it CROSSES THE API BOUNDARY typed … the FE switches on the discriminant"*); there was no such FE. **PLACED UNDER MONITOR, not Manage**, and the choice is worth stating because the events are all mutations of MANAGE resources: the hubs split by ACT — Manage is where you change the workspace, Monitor is where you read what has happened to it — and the overview's T13 line files this surface under monitor surfaces. It is the third answer to "what happened", beside Runs (what an execution did) and AI activity (what the models did). `describeWorkspaceEvent` is **exhaustive by construction** (a `never` assertion in the default), so a sixth variant added to the union fails TYPECHECK rather than rendering an "unknown event" placeholder — an audit surface that can silently under-report an act is worse than one that does not build. Wording is held to the schema's own semantics, which are easy to paraphrase into a lie: a RESTORE re-enables nothing (reusing the product's existing `TRIGGERS_STAY_DISABLED_NOTE` rather than minting a second sentence for the same fact), an IMPORT states its own archives because they are deliberately not double-counted as separate `pipeline.archived` events, and a first publish reads "the first publish" rather than interpolating its `null` `from`. Read-only, so `usePolledResource` and not `useGuardedLoad` — the two have OPPOSITE drop rules and swapping them is silent — with no `intervalMs`, because the log only moves when the operator acts elsewhere. *(Both SUPERSEDED by #1076 below: the hook is now `usePagedList`, and the order comes from the server.)* Ordering is **newest-first by reversing the ascending walk**, exact rather than approximate since `seq` is monotonic per owner; no re-sort on `createdAt`, whose wall clock is not the log's ordering authority. The client parse is deliberately ALL-OR-NOTHING: a row that cannot be read fails the load loudly, because a silently partial history reads exactly like a complete one. An `import.applied` counts and names only what the import actually WROTE — `applied` is the apply's full judgement and carries `unchanged`/`superseded` rows, so the existing `appliedActionWroteNothing` predicate is reused (ORed with the orthogonal `versionMinted`, exactly as `buildImportAppliedEvent` does) rather than re-spelled; the resources that already matched are DISCLOSED as a count instead of dropped, because "considered, nothing to do" and "not considered" are different facts. **DEFERRED: #1076** (the log is walked in full to render its newest page — it wants server-side descending order plus an incremental "load older") and **#1077** (a published pipeline renders as a bare `resourceId`: naming it needs BOTH the live and the archived pipeline lists — an audit log is disproportionately about pipelines since archived — which is two more requests and two more failure modes on a read-only page, and it has a real question inside it, since the name NOW is not the name at publish time). | Monitor |
| | **AS BUILT (2026-08-13, #1076 — the audit log stops being walked to be read).** #1075's page rendered its newest twenty entries by fetching EVERY page of an append-only log and reversing it client-side, because `GET /api/workspace/audit` had no descending mode. Bounded then (the log grows at AUTHORING rate — a publish, an archive, an import — at one request per 100 rows) but it is the first list in the app with **no retention policy at all**, so that cost rises for the life of a workspace and never falls. The route is now bidirectional (`?order=asc\|desc`) and the page consumes `Paginated<T>` directly with a **"Load older entries"** control. **The DESC pair was pre-designed, not invented here:** `pageOrder`'s docblock had already stated that a newest-first surface would need a `desc`/`lt` sibling of BOTH it and `afterCursor`, and that the cursor codec and `toPage` were direction-agnostic so only those two helpers would gain a direction — `beforeCursor` + `pageOrderDesc` are exactly that, leaving ONE cursor format. The ordering scalar stays `seq` (the per-owner append order) rather than the wall-clock `createdAt`, so "newest" is exact for two events minted in the same millisecond. **The route default stayed `asc`.** Flipping it was considered and rejected: `/api/runs` looked like the newest-first precedent and is not (it is not keyset-paginated at all), the one consumer must send `order=desc` on every page ANYWAY — a cursor names a position and carries no direction, so mixing them returns a coherent-but-wrong slice with nothing to error on — and a flipped default would have silently re-sliced the log under the append-order readers (import provenance, the publish-history assertions) to save one query parameter at a single caller. **`usePagedList` is a THIRD load shape, and it does not hand-roll a fourth latest-wins counter:** it wraps the existing `useGuardedLoad` runner, which already owns the counter, the mount abort and the two-argument `.then`. That reuse gets the invariant right by construction — ONE counter, because there is ONE state target, so a refresh VOIDS an in-flight "load older" (that older page was computed against a head the refresh has replaced; appending it after would splice two different reads of the log together). A refresh REPLACES the head and discards the tail rather than blanking, so `items === null` keeps its single meaning of "nothing has loaded YET" and the empty-log sentence can never be printed about a request rather than about the workspace. Two failures are worded apart because they are different news — a failed first page means there is no history on screen, a failed older page means the history on screen is real and merely stops short — and the empty-log notice is suppressed while either is showing, since both sentences can be true at once and together the reader cannot tell which is the news. `AUDIT_PAGE_SIZE` is 25 and deliberately neither `DEFAULT_PAGE_SIZE` nor `MAX_PAGE_SIZE`: those size a transport chunk for a caller reconstructing a whole list, where bigger is strictly better, whereas this is how much history a reader sees before asking for more. The e2e spec asserts the two things only a browser can — the newest entry is the FIRST row on first paint, and an entry buried past the page boundary is not missing but one click away — which is safe against the log's shared-run accumulation only because `playwright.config.ts` runs `workers: 1, fullyParallel: false`; that reasoning replaces the #1075 spec's blanket "asserts nothing about ordering" rule, which had been written as if concurrency were the risk. | Monitor |
...
| Route | Renders |
|---|---|
| `#/` | Home hub — recent runs + hub shortcuts (U15 slice 1; Settings is slice 2) |
...
| `#/settings` | Settings — master-key status + theme (U15 slice 2, #1094). A SIBLING of the hubs, not a section inside one: it carries no `hub` handle, so no secondary pane renders and no rail hub lights up. Reached from the gear in the rail FOOT. |

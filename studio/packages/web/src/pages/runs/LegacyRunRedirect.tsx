 ...
 *
 * The id is RE-ENCODED on the way out, via the same `runDetailPath` helper
 * every other run-path builder uses. `useParams` returns the id already decoded
 ...
 * `RunDetailRoute` then decodes a SECOND time. `routes.test.tsx` pins this with
 * an id that is not idempotent under an extra decode — a plain id would let the
 * bug through.
 *
...
export function LegacyRunRedirect() {
  ...
  return <Navigate to={runDetailPath(runId)} replace />;
}

...
_(Since closed: item 4 by #1196, which
...
end — `.columns()` in `sqlite.ts`, the statement describe in `postgres.ts`; item 3 as below.)_
...
(on the table or any partition), an `INSERT` rule, or a foreign-table partition. Those shapes
...
postgres rung names them: _"'gen' is a generated column"_. Same rung, better sentence.
...
`Number.MAX_SAFE_INTEGER`) makes `JSON.stringify` THROW _inside the insert transaction_, so the
...
whose own docblock calls itself _"a UX PRE-CHECK, never the gate"_, and the adapter's dispatch parse.
...
| **M8**  | The mapping authoring panel (§13). **SPLIT IN TWO** — slice 1 (#1169) is the `objectList` PRIMITIVE (§13's _"build it as a primitive rather than a copy-specific panel"_): the derived row control, which upgrades `copy.mapping` and `llm_call.tools`. Slice 2 is the copy-specific half — Auto-map, the explicit _unmapped_ state and per-column expressions — all three of which need a sink-column seam that does not exist yet. See §13's as-built block                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | UI epic; e2e-gated                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**So the reader is BUILT, on M7's precedent** (which discharged its own _"a CSV parser"_ row by
...
that it _"falls through to the JSON control"_ was wrong when written. `llm_call`'s
...
`z.array(...)` but `validateDoc` refuses any non-string value — _"history must be
a whole-value `${...}` expression"_ — so in every valid doc it holds a STRING.
...
laziness.** Auto-map (§6.3) and the explicit _unmapped_ state both need the SINK's
...
_unmapped_ state "both need the SINK's column list, and there is no seam that resolves one at
...
**The _unmapped_ state is an ADVISORY, not a persisted acknowledgment.** §13 asks that a column

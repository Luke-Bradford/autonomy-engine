...
describe('undo/redo control state (U17)', () => {
  it('a live stack gives no reason — the control is enabled', () => {
    expect(undoDisabledReason({ available: true, previewing: null })).toBeNull();
    expect(redoDisabledReason({ available: true, previewing: null })).toBeNull();
  });

  it('an empty stack says so', () => {
    expect(undoDisabledReason({ available: false, previewing: null })).toBe('Nothing to undo.');
    expect(redoDisabledReason({ available: false, previewing: null })).toBe('Nothing to redo.');
  });

  it('a preview takes the controls away, and says which document it means', () => {
    expect(undoDisabledReason({ available: true, previewing: 3 })).toMatch(/working graph/);
    expect(redoDisabledReason({ available: true, previewing: 3 })).toMatch(/working graph/);
  });

  it('the preview reason WINS over the empty-stack one — it is the actionable half', () => {
    expect(undoDisabledReason({ available: false, previewing: 3 })).toMatch(/Leave the preview/);
  });
});

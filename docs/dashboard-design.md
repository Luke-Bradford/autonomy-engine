# Autonomy control-room dashboard — design

> **Build direction (operator, 2026-07-01): the mockups were rough shape only — build this
> PRODUCTION-GRADE and CLEAN, not a wireframe.** The panel breakdown below is the agreed *structure*;
> the *polish* is a build task. When building the page:
> - Use the `frontend-design` skill. Cohesive theme, deliberate typography + spacing, real visual
>   hierarchy. NO generic AI-slop (no Inter/Roboto default, no purple-gradient-on-dark, no
>   cookie-cutter card grid). It should look like a considered product, not a scaffold.
> - Single self-contained local page, localhost-bind only. Prefer stdlib server + SSE (no heavy
>   deps — matches the engine's no-dependency posture), one HTML page, no build step.
> - **Build order:** **P1** read-only live view (the visibility win alone: repos list, live activity
>   tree, supervisor voice, git-in-flight, tokens) → **P2** control surface (start / graceful-stop /
>   hard-stop, settings + point-at-repo, live model/effort override). Graceful-stop needs the
>   supervisor PAUSE-sentinel (a separate small engine change — see the backlog issue).
> - Every field traces to data the engine already emits (stream-json session logs, `supervisor.log`,
>   `.last_usage_reset`, `gh`, git state, `.autonomy/config.yaml`). The page EXPOSES; it does not
>   invent telemetry. Render hand-rolled-engine events and (future) Managed-Agents events with one
>   visual language via the activity-source abstraction.
> - The rough mockups from the design session are not committed (ephemeral). This doc is the spec.

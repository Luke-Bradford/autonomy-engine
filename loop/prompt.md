...
4c. **NEVER END YOUR TURN WITH A BASH TASK STILL RUNNING (#1261).** You are `claude -p`. A pending
    ...
    in the FOREGROUND on its output (`until grep -q <done-marker> <file> || ! kill -0 <pid> 2>/dev/null; do
    sleep 5; done` — the `kill -0` exit stops a dead task from spinning you until the timeout; split
...
4d. **NEVER TOUCH ANOTHER PROJECT'S CONTAINERS OR DATABASES (#1271).** This machine also runs the
    ...
    container you did not start in this fire — anything `ebull-*` above all. That goes for "harmless"
    DDL too: fire 11 on 2026-09-24 ran `create function … / drop function fill() cascade` inside
    `ebull-postgres-test` with `ON_ERROR_STOP=0`, so had eBull owned a `fill()`, a failed create
    followed by that drop would have destroyed eBull's function and everything depending on it.
    When you need a real database, START YOUR OWN throwaway container named `studio-<issue>-…` AND
    labelled `studio-loop=1` (e.g. `docker run -d --rm --label studio-loop=1 --name studio-1162-pg
    -e POSTGRES_HOST_AUTH_METHOD=trust -p 127.0.0.1:<free port>:5432 postgres:17`), and
    `docker stop` it explicitly before the fire ends — `--rm` only cleans up once it stops, so a
    fire that dies mid-run leaks it. The ONE exception to "did not start in this fire": a container
    carrying the `studio-loop=1` label is the loop's own leftover, and you may stop it
    (`docker ps -q --filter label=studio-loop=1`). Never widen that filter to a name pattern.

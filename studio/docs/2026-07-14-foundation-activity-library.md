...
| # | Ticket |
| --- | --- |
| **A0** | **Branch/outcome model** (PREREQUISITE). **The `Edge` union is now BUILT in #1 (F1)** — `BranchEdgeSchema` = `{on:'branch', branch:'<name>'}` (label required; may also be a capped `back:true` arm), operational outcomes stay `success/failure/skipped/completion`. A0 no longer designs the schema; it must (a) implement `if`→`true`/`false` and `switch`→named case/default against it, (b) carry the chosen branch label into run state via `condition.evaluated`/`switch.evaluated` (#1 F1 deliberately did NOT invent a rival mechanism — `edgeState` currently returns `unsatisfied-terminal` for every branch edge and `settle` emits a diagnostic saying so), and (c) REPLACE `validateDoc`'s blanket save-time refusal of branch edges with the real rule ("a branch edge's source must declare that branch"), which needs the ActivityDefinition contract. |
